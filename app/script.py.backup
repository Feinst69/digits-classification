...
# Importer la configuration et le modèle
from config import ensure_directories, PROJECT_ROOT, APP_DIR, STATIC_DIR, TEMPLATES_DIR, UPLOAD_DIR, TEMP_DIR, BEST_MODEL_PATH
from src.CNN_MODEL import CNN_MODEL

# S'assurer que tous les répertoires existent
ensure_directories()

# Créer l'application Flask avec la configuration correcte
app = Flask(__name__,
...
# Charger le modèle
print(f"Chargement du modèle depuis: {BEST_MODEL_PATH}")
cnn_model = CNN_MODEL(BEST_MODEL_PATH)
    ...
    try:
        if 'file' in request.files:
            ...
            if file.filename != '':
                ...
                # Faire la prédiction
                result = cnn_model.get_prediction_for_web(image_path=filepath, temp_folder=TEMP_DIR)
...
if __name__ == '__main__':
    ...
    print(f"Répertoire de l'app: {APP_DIR}")
